...
def lyGenerator(aid, adminkey, env="prod"):
    ...
    if r.status_code == 200:
        ...
        print("failed to create ly object with status code: {}".format(r.status_code))
...
class ly:
    def __init__(self, key, env="prod"):
        ...
        r = requests.get(self.API+"api/account", params={"key": key})

        if r.status_code != 200:
            raise Exception(
                "failed to create ly object with status code: {}".format(r.status_code))

        data = json.loads(r.text)
        self.aid = data["data"][0]["aid"]
